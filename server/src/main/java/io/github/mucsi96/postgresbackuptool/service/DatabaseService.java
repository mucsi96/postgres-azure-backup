...
public class DatabaseService {
    ...
    private File executePgDump(String databaseName, String format,
        ...
        List<String> baseArgs = new ArrayList<>(List.of("pg_dump", "--schema",
        ...
        try {
            int status = createPostgresProcess(commands, databaseConfiguration)
                    .start().waitFor();
    ...
    public void restoreDump(String databaseName, File dumpFile)
        ...
        try {
            ...
            int psqlStatus = createPostgresProcess(
                    List.of("psql", "--single-transaction",
                            "--variable=ON_ERROR_STOP=1", "--file",
                            combinedSql.getAbsolutePath()),
                    databaseConfiguration).start().waitFor();
            if (psqlStatus != 0) {
                throw new RuntimeException(
    ...
    private ProcessBuilder createPostgresProcess(List<String> command,
        ...
        processBuilder.environment().put("PGHOST",
                databaseConfiguration.getHost());
        processBuilder.environment().put("PGPORT",
                Integer.toString(databaseConfiguration.getPort()));
        processBuilder.environment().put("PGDATABASE",
                databaseConfiguration.getDatabase());
        processBuilder.environment().put("PGUSER",
