 ...
 *
 * The profile condition is evaluated during AOT processing for the GraalVM
 * native image build, so the bean only exists in images built with a
 * non-test profile (see the SPRING_PROFILES_ACTIVE build arg in the
 * Dockerfile).
 */
@Component
@Profile("!test")
