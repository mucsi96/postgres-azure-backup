...
helm upgrade $SERVER_RELEASE_NAME mucsi96/spring-app \
    ...
    --set persistentVolumeClaims[2].name=training-log-backup-pvc \
    --set persistentVolumeClaims[2].accessMode=ReadWriteOnce \
    --set persistentVolumeClaims[2].volumeName=training-log-backup \
    --set persistentVolumeClaims[2].mountPath=/app/storage/training-log \
